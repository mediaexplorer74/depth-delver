...
public class AnimationController : IAnimationController
{
    ...
    public int CurrentFrame
    {
        get
        {
            return _definition.Frames[_internalFrame].FrameIndex;
        }
    }
    ...
    public void Update(GameTime gameTime)
    {
        ...
        if (!IsAnimating || IsPaused)
        {
            // Fix for CS0019: Convert the TimeSpan to a double (total seconds), perform the multiplication, and convert back to TimeSpan.
            CurrentFrameTimeRemaining -= TimeSpan.FromSeconds(elapsedTime.TotalSeconds * Speed);
            return;
        }
